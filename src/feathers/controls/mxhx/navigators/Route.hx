...
class Route extends feathers.controls.navigators.Route implements IMXHXNavigatorItem {
	...
	/**
		If `true`, and `Route.viewFactory` is `null`, a fallback view will
		be created. If `false`, then no view will be created and the navigator
		will receive `null`.
	**/
	public var fallbackViewFactoryEnabled:Bool = false;

	override private function getView(navigator:RouterNavigator):DisplayObject {
